...
for power = power_min:power_step:power_max
    for gain = gain_min:gain_step:gain_max
        ...
        for freq_ind = 1:size_freq
            %
            ILpp = Vo*(1-gain)/(L*freq(freq_ind));
            %
...
for i=1:power_number
    for j=1:gain_number
        ...
        if sum(results((i-1)*gain_number+j,1:CapLossCol)) > 80
...
figure
h = heatmap(power_min:power_step:power_max,gain_min:gain_step:gain_max,results_hmp_loss,'ColorLimits',[0 80]);
xlabel('Output Power');
ylabel('Gain')
title('Power Loss')
...
function [CapLoss] = CapLossCalculate(ILavg,ILpp,fsw,duty)
    ...
    Icap_th = 0;%680 / (680 + 2*220) * Icaprms; %single th capacitor
    Icap_smd = 1 * Icaprms ; %single smd capacitor
